...
namespace Doug.Effects
{
    ...
    public class EventDispatcher : IEventDispatcher
    {
        ...
        {
            var equipment = new List<EquipmentItem>(user.Loadout.GetEquipmentList());
        ...
        {
            return PropagateItemEvents(user, true, (isDead, item) => item.Effect.OnDeath() && isDead);
        }

        public void OnDeathByUser(User user, User killer)
        {
            var equipment = new List<EquipmentItem>(user.Loadout.GetEquipmentList());
            equipment.ForEach(equip => equip.Effect.OnDeathByUser(killer));
        }
        ...
        {
            slur = PropagateItemEvents(target, slur, (acc, item) => item.Effect.OnGettingFlamed(command, acc));
            slur = PropagateEffectEvents(target, slur, (acc, effect) => effect.OnGettingFlamed(command, acc));

            return PropagateItemEvents(caller, slur, (acc, item) => item.Effect.OnFlaming(command, acc));
        }

        public double OnGambling(User user, double baseChance)
        {
            return PropagateItemEvents(user, baseChance, (chance, item) => item.Effect.OnGambling(chance));
        }

        public double OnStealingChance(User user, double baseChance)
        {
            return PropagateItemEvents(user, baseChance, (chance, item) => item.Effect.OnStealingChance(chance));
        }

        public double OnGettingStolenChance(User user, double baseChance)
        {
            return PropagateItemEvents(user, baseChance, (chance, item) => item.Effect.OnGettingStolenChance(chance));
        }

        public int OnStealingAmount(User user, int baseAmount)
        {
            return PropagateItemEvents(user, baseAmount, (amount, item) => item.Effect.OnStealingAmount(amount));
        }

        public string OnMention(User user, string mention)
        {
            return PropagateItemEvents(user, mention, (amount, item) => item.Effect.OnMention(amount));
        }
    }
}
